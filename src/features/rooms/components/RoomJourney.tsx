...
export function RoomJourney({ palaceId, roomId, roomTitle, palaceTitle, mode, nodes }: Props) {
  ...
  if (!current) {
    return (
      <div className="-mx-4 -my-6 flex min-h-[calc(100svh-3.5rem-3rem-var(--height-bottom-nav)-env(safe-area-inset-top)-env(safe-area-inset-bottom))] flex-col items-center justify-center gap-4 p-8 text-center sm:-mx-6 md:min-h-svh lg:-mx-8">
  ...
  return (
    <div className="-mx-4 -my-6 flex h-[calc(100svh-3.5rem-3rem-var(--height-bottom-nav)-env(safe-area-inset-top)-env(safe-area-inset-bottom))] flex-col bg-background sm:-mx-6 md:h-svh lg:-mx-8">
      {}
      ...
      {}
      <main className="flex flex-1 items-start justify-center overflow-y-auto overscroll-contain px-4 py-6 sm:items-center sm:px-6 sm:py-10">
        <m.article
          key={current.id}
          drag={swipe.drag}
