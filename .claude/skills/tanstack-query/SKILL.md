...
const { data } = useQuery({
  ...
  queryFn: ({ signal }) => axios.get(`/api/repos/${username}`, { signal }).then((res) => res.data),
});
```
...
queryClient.removeQueries({
  predicate: (query) => query.queryKey[0] === 'todos' && query.state.data?.isArchived === true,
});
```
...
const { error } = useQuery({
  ...
  queryFn: async () => {
    ...
    if (!response.ok) {
      throw new APIError('Failed to fetch user', response.status, await response.text());
    }
    return response.json();
  },
});
...
export function useCreateUser() {
  ...
  return useMutation({
    mutationFn: (newUser: NewUser) => apiClient.post('/users', newUser).then((res) => res.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['users'] });
    },
  });
...
function UserLink({ userId }: { userId: number }) {
  ...
  return (
    <Link href={`/users/${userId}`} onMouseEnter={prefetchUser} onFocus={prefetchUser}>
      View User
    </Link>
  );
}
...
export function createResourceHooks<T, CreateT = Partial<T>, UpdateT = Partial<T>>(
  ...
  return {
    ...
    useUpdate: () => {
      const queryClient = useQueryClient();
      return useMutation({
        mutationFn: ({ id, data }: { id: string | number; data: UpdateT }) => api.update(id, data),
        onSuccess: (_, { id }) => {
