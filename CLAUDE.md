...
**Current status:** Phases 1, 2 (2A/2B/2C), 3A (schema), 3B (RLS + server actions), and 3C (rate limiting, FTS search, cursor pagination) are complete. Canvas, CRDT, Zustand, TanStack Query — not started.
...
Feature dirs in `apps/web/src/features/<domain>/` are created **only when work starts**. `auth/`, `dashboard/`, `palaces/`, and `nodes/` exist today. Cross-feature imports are forbidden by `eslint-plugin-boundaries`; cross-cutting code goes to `src/shared/`.
