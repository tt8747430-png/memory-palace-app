...
- **Dashboard full-bleed math** — the dashboard `<main>` wraps every page in `<div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">`. Negative margins (`-mx-4 -my-6 sm:-mx-6 lg:-mx-8`) **shift** content visually but do **not** shrink the wrapper's box — its `py-6` still adds 3rem to its own height. Any explicit `h-[calc(...)]` on the inner div must therefore subtract that `3rem` in addition to the mobile header (`3.5rem`), bottom nav (`var(--height-bottom-nav)`), and `env(safe-area-inset-bottom)`. Canonical formula for full-bleed pages: `h-[calc(100dvh-3.5rem-3rem-var(--height-bottom-nav)-env(safe-area-inset-bottom))] md:h-dvh`. Forgetting the `3rem` pushes content under the fixed `BottomNav` on mobile — the visible symptom is a phantom scroll on iPhone 14 Pro (**402×874 layout viewport** — the reference device for mobile sanity checks) and floating canvas affordances (CanvasFab pill, React Flow controls) hiding behind the bottom nav. The room canvas page, `RoomJourney`, and `FlashcardDeck` are the three canonical implementations.
