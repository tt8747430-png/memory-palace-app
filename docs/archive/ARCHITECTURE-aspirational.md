...
| Layer                | Tool                                                               | Purpose                                                            |
| -------------------- | ------------------------------------------------------------------ | ------------------------------------------------------------------ |
| **Framework**        | Next.js 16.2.4 (App Router) + React Compiler                       | React foundation, routing, Server Actions, SSR, auto-memoisation   |
...
| **Styling**          | Tailwind CSS v4 + shadcn/ui                                        | Accessible, customizable design system                             |
| **i18n**             | next-intl                                                          | Internationalisation, server + client message loading              |
...
├── scripts/
│   └── ci/
│       ├── check-guardrails.mjs        # Enforces proxy.ts pattern (no middleware.ts)
│       └── validate-vercel-config.mjs
│
├── apps/
│   └── web/                            # Next.js 16.2.4 Application
│       ├── src/
│       │   ├── app/                    # App Router routes
│       │   │   ├── (auth)/login/page.tsx
│       │   │   ├── (auth)/signup/page.tsx
│       │   │   ├── (auth)/callback/route.ts  # Supabase OAuth code exchange
│       │   │   ├── (dashboard)/layout.tsx    # Auth-guarded layout
│       │   │   ├── (dashboard)/page.tsx
│       │   │   ├── palace/[palaceId]/room/[roomId]/page.tsx  # FUTURE (Phase 5)
│       │   │   ├── layout.tsx
│       │   │   └── error.tsx
│       │   ├── features/
...
│       │   │   ├── dashboard/          # Shell, Sidebar, BottomNav, MobileDrawer
│       │   │   └── 3d-room/            # FUTURE: React Three Fiber (post-v1.5.0)
│       │   ├── shared/
...
│       │   └── proxy.ts                # Next.js 16.2.4 middleware (session refresh + route guard)
│       ├── messages/
...
> **Note on middleware:** Next.js 16.2.4 changed how middleware is handled. This project uses `src/proxy.ts` as the middleware entrypoint — **not** `src/middleware.ts`. The CI guardrail (`check-guardrails.mjs`) fails the build if `middleware.ts` exists alongside `proxy.ts`.
...
- `h-dvh` — dynamic viewport height handles mobile browser chrome correctly (see [UI_STYLE_GUIDE.md §1](./UI_STYLE_GUIDE.md#1-mobile-first-design-strategy))
- `pb-[calc(4rem+env(safe-area-inset-bottom))]` — main content scrolls above the bottom nav + iOS home indicator
- `pb-safe-bottom` — bottom nav pads for the iOS home indicator (Tailwind v4 CSS custom property shorthand)
